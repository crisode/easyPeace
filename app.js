...
http.createServer((req, res) => {
	...
	let orden = movies.sort(function (a, b) { // creo una variable llamada orden que va a contener el objeto movies ordenado por el metodo .sort  
		if (a.title > b.title) { // comparo los titulos 
			return 1;
		}
		if (a.title < b.title) { // vuelvo a comparar los titulos 
			return -1;
		}
		return 0;
	});
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write(`El total de peliculas en cartelera en estos momentos : ${movies.length}\n\n`)
			res.write("Nuestro catelogo de peliculas incluye : \n\n ")

			orden.forEach(movies => { // utilizo el forEach para recorrer el la variable "orden" que contiene el array de objetos literales ordenados alfabeticamente 
				let az = movies.title // creo una variable que va a contener los datos de title del objeto movies 
				res.write("Pelicula:" + az + "\n")
		...
		case '/en-cartelera':
			...
			res.write(`El total de peliculas en cartelera en estos momentos : ${movies.length}` + " peliculas\n\n")
			movies.forEach(movies => { //recorro el objeto y tomo los datos de title y overview y los coloco en una variavle para despues llamar a esa variable "detalleCartelera"
				let detalleCartelera = movies.title.toUpperCase() + ":\n\n" + movies.overview + "\n\n"
		...
		case '/mas-votadas':
			...
			let votadas = movies.filter(function(movies){
				return movies.vote_average >=7
			}) 
			votadas.forEach(movies => {
				return( " pelicula " + movies.title +  "\n") 
			});
			res.write("-------------------------------------\n")
			res.write("el total de peliculas con mejor rating es de: " + `${votadas.length}`+"\n\n")
			
		      let rating = votadas.filter(funtion(movies){
				  return 
			  })
			...
			movies.forEach(movies => {
				let listadoDePeliculas = movies.title + " Puntaje " + movies.vote_average + "\n\n" + "Reseña: " + movies.overview + "\n\n"
				res.write(listadoDePeliculas)
			}); 
		...
		case '/sucursales':
			...
			res.write(`El total de nuestras salas en estos momentos es de : ${theaters.length}`+ "\n\n")
			theaters.forEach(theaters => {
				let listadoDesalas = "\n\nSala : " + theaters.name + "\n\n" +"Direccion : "+  theaters.address + "\n\n" + "Descripcion : " + theaters.description + "\n\n" + "-----------------------------------------------------------------------"
				res.write(listadoDesalas)
			});
